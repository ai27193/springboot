package com.ou.springbootmybatis.realm;//package com.example.shirojwt.shiro;

import com.ou.springbootmybatis.entity.User;
import com.ou.springbootmybatis.service.impl.UserServiceImpl;
import com.ou.springbootmybatis.token.JwtToken;
...
public class MyRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        try {
            //添加角色和权限
            Set<String> listRole= userServiceImpl.selectByNameRole(username);
            simpleAuthorizationInfo.addRoles(listRole);
            for (String role : listRole) {
                Set<String> permission= userServiceImpl.selectByNamePermission(role);
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        User userBean = userServiceImpl.selectByName(username);
        if (userBean == null) {
            log.error("用户不存在!)");
